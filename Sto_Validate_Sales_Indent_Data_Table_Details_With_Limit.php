...
include '../auto_load.php';
	...
	$Dcode=$_SESSION['Dcode'];
	 ...
	 $sql="EXEC Sales_Indent_STO_Details_WIth_Limit_STO @Emp_Id='".$Emp_Id."',@Product_Division='".$ProductDivision."',@Dcode='".$Dcode."',@plant_id='".$Plant_Id."',@Zone_id='".$Zone_id."',@Region_Id='".$Region_Id."',@Terrirory_Id='".$Terrirory_Id."',@Status='1',@Length='$length',@Offset='$offset'";
	...
	{		
		if(@$prow['Validate_Permission_With_In_Limit'] == 1){
			...
			$resarr[] 	= '<input type="text" class="form-control QtyInPkt textbox-grey" name="QtyInPkt['.@$prow['Id'].']" value="'.round(@$prow['QtyInPkt'],2).'"readonly><input type="hidden" class="MaterialQtyInPkt" value="'.round(@$prow['MaterialQtyInPkt'],2).'"readonly>';
			$resarr[] 	= '<input type="text" class="form-control QtyInKg textbox-grey" name="QtyInKg['.@$prow['Id'].']" value="'.round(@$prow['QtyInKg'],2).'"readonly><input type="hidden" class="MaterialQtyInKg" value="'.round(@$prow['MaterialQtyInKg'],2).'"readonly>';
			if(@$prow['Validate_Permission_With_In_Limit'] == 1){
