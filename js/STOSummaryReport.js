  $(document).ready(function(){
   ...
   $(".region_wise_summary_div,.vareity_wise_summary_div,territory_wise_summary_div").css("display","none");
      ...
      // $(".region_wise_summary_div,.vareity_wise_summary_div").css("display","none");
  ...
  {
   var division_id=$(this).closest("tr").find(".table_zone_id").val();
   var status=$(this).closest('tr').find(".summary_status").val();
   var product_division_id=$(".product_division_id").val();
   ...
   if(status == "0"){
    ...
    region_wise_summary_details("yes","Get_Region_Wise_Summary",product_division_id,division_id,region_id,plant_code,hybrid_id,crop_id,indent_type_id);
}else{
    $(this).closest('tr').find(".summary_status").val(0);
 ...
 function zone_wise_summary_details(destroy_status, Action, product_division_id, ZoneId, RegionId, CropCode) {
    ...
    $('.zone_wise_summary').DataTable({
        "columnDefs": [{
            ...
            "className": "popup_sto_indent td_align",
            "targets": 1
        }],
        /* Footer Start Here */
        ...
        "ajax": {
            "url": "Sto_Indent_Summary_Ajax.php",
            "type": "POST",
            "data": {
                Action: Action,
...
$(document).on("click",".popup_sto_indent",function(){
    var product_division_id=$(".product_division_id").val();
    ...
    if(from == 'zone') {
        ...
        var crop_id = $(this).closest('tr').find('.table_crop_id').val();
        var material_code = $(this).closest('tr').find('.table_variety_id').val();
        req_obj.crop_id = crop_id; 
      ...
      success: function(data){
        $('#indent_summary_tbl').DataTable().destroy(); 
         ...
         data.forEach(function(val){
            ...
            <td>${ (val.PlantName !== null) ? val.PlantName : '' }</td>
