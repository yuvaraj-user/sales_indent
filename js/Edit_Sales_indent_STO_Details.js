  ...
  if(confirm('Are You Sure Want to Delete This?')){
    if($(".remove_btn").length >1)
    {
     ...
     ({
      type: "POST",
      url: "Common_Ajax.php",
      data:{"Action":"Delete_STO_Material_Details","SalesIndentId":SalesIndentId},
      async:false,
...
function get_material_details(product_division,crop_code,zone_id,status,Material_Code){
  var output="";
  $.ajax 
  ({
    type: "POST",
    url: "Common_Ajax.php",
    data:{"Action":"Get_Material_Details","product_division":product_division,"crop_code":crop_code,"zone_id":zone_id,"Material_Code":Material_Code},
    async:false,
    success: function(data){
     result=JSON.parse(data);
     output=result.data;
...
$(document).on("change",".product_division,.crop_id,.zone_id",function(){
  ...
  get_material_details(product_division,crop_code,zone_id,0,0);
